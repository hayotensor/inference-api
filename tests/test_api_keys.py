...
async def test_api_key_creation_listing_revocation_and_product_routes(client, monkeypatch):
    ...
    models = await client.get("/v1/models", headers={"Authorization": f"Bearer {key_body['key']}"})
    ...
    wildcard_runtime = await client.get("/v1/models", headers={"Authorization": f"Bearer {key_body['key']}"})
    assert wildcard_runtime.status_code == 403
    async with async_session_maker() as db_session:
        db_key = await db_session.get(APIKey, UUID(key_body["id"]))
        db_key.scopes = ["models:read", "inference:write", "usage:read"]
        await db_session.commit()

    inference = await client.post(
        "/v1/inference",
        headers={"X-API-Key": key_body["key"]},
        json={"prompt": "hello platform"},
    )
    assert inference.status_code == 200

    usage = await client.get("/v1/usage", headers={"Authorization": f"Bearer {key_body['key']}"})
    ...
    rejected = await client.get("/v1/models", headers={"Authorization": f"Bearer {key_body['key']}"})
...
async def test_api_key_expiration_and_scope_validation(client, monkeypatch):
    ...
    async with async_session_maker() as db_session:
        ...
        await db_session.commit()
    expired_key = valid_body["key"]
    rejected = await client.get("/v1/models", headers={"Authorization": f"Bearer {expired_key}"})
    assert rejected.status_code == 401
    ...
    allowed = await client.get("/v1/models", headers={"Authorization": f"Bearer {scoped_key}"})
