...
API-only FastAPI backend for an OpenAI-style developer platform. It includes revocable user JWT auth, refresh-token rotation, OAuth callbacks, SMS OTP phone auth, custom API keys, product API examples, Alembic migrations, Postgres, Redis-backed rate limiting, and pytest coverage.
